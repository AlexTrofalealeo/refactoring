...
public class Tree {
    ...
    public void setLocation(String locationLatitude, String locationLongitude, String locationName){this.location = new Location(locationLatitude, locationLongitude, locationName);}
    @Override
    ...
    void addAppraisal(LocalDateTime appraisalDate) {this.appraisalDates.add(appraisalDate);}
    public List<LocalDateTime> getAppraisals(){return this.appraisalDates;}
    public boolean isNextAppraisalOverdue(){
        ...
        if (this.appraisalDates.size() > 0) {latestAppraisalDate = this.appraisalDates.get(0);}
        for(LocalDateTime appraisalDate : this.appraisalDates) {
            if (latestAppraisalDate.isBefore(appraisalDate)) {latestAppraisalDate = appraisalDate;}
        }
